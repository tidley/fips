...
use crate::{
    build_punch_packet, create_traversal_answer, decode_session_frame, parse_punch_packet,
    LegacyHelloMessage, LegacyPunch, LegacyServerInfoMessage, LegacyStunInfo, PunchHint,
    PunchPacketKind, TraversalOffer,
};
            ...
            loop {
                ...
                if let Ok(punch) = parse_punch_packet(packet) {
                    ...
                    if let Some(session_id) = session_id {
                        ...
                        let _ = self.ensure_session(&session_id, remote).await;
                        continue;
                    }
                }

                if let Ok(frame) = decode_session_frame(packet) {
                    let session = self.ensure_session(&frame.session_id, remote).await;
