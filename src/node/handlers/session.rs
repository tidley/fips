...
impl Node {
    ...
    async fn handle_encrypted_session_msg(
        ...
        match SessionMessageType::from_byte(msg_type) {
            Some(SessionMessageType::DataPacket) => {
                ...
                match dst_port {
                    ...
                    _ => {
                        self.deliver_app_datagram(
                            src_addr,
                            src_port,
                            dst_port,
                            service_payload.to_vec(),
                        );
                    }
                }
            }
        ...
        self.flush_pending_app_packets(src_addr).await;
    }
    ...
    /// Send application data over an end-to-end session, establishing one if needed.
    pub async fn send_app_data_to_npub(
        &mut self,
        peer_npub: &str,
        src_port: u16,
        dst_port: u16,
        ...
        let peer_identity =
            PeerIdentity::from_npub(peer_npub).map_err(|e| NodeError::InvalidPeerNpub {
    ...
    /// Send application data to a known peer node address.
    pub async fn send_app_data(
        ...
        if dst_port == FSP_PORT_IPV6_SHIM {
            return Err(NodeError::AppPortReserved(dst_port));
        }

        if let Some(entry) = self.sessions.get(&dest_addr) {
            if entry.is_established() {
                return self
                    .send_session_data(&dest_addr, src_port, dst_port, payload)
                    .await;
            }

            self.queue_pending_app_packet(dest_addr, src_port, dst_port, payload.to_vec());
            return Ok(());
        }
        ...
        if let Err(e) = self.initiate_session(dest_addr, dest_pubkey).await {
            debug!(
                ...
                error = %e,
                "Failed to initiate app session, trying discovery"
            );
    ...
    /// Queue an application datagram while waiting for session establishment.
    fn queue_pending_app_packet(
        &mut self,
        dest_addr: NodeAddr,
        src_port: u16,
        dst_port: u16,
        ...
        let max_dests = self.config.node.session.pending_max_destinations;
        if !self.pending_app_packets.contains_key(&dest_addr)
            && self.pending_app_packets.len() >= max_dests
        {
            return;
        }

        let queue = self.pending_app_packets.entry(dest_addr).or_default();
        if queue.len() >= self.config.node.session.pending_packets_per_dest {
            queue.pop_front();
        }
        queue.push_back(crate::app::PendingAppPacket::new(
            src_port, dst_port, payload,
        ));
    }

    /// Flush pending application datagrams for a destination whose session just became established.
    async fn flush_pending_app_packets(&mut self, dest_addr: &NodeAddr) {
        let packets = match self.pending_app_packets.remove(dest_addr) {
            Some(q) => q,
            None => return,
        };
        ...
        for packet in packets {
            if let Err(e) = self
                .send_session_data(dest_addr, packet.src_port, packet.dst_port, &packet.payload)
                .await
            {
                debug!(
                    dest = %self.peer_display_name(dest_addr),
                    error = %e,
                    "Failed to send queued app packet"
                );
                break;
            }
        }
    ...
    fn deliver_app_datagram(
        ...
        let sender = match self.app_ports.get(&dst_port) {
        ...
        let peer_npub = if let Some(entry) = self.sessions.get(src_addr) {
            let (xonly, _) = entry.remote_pubkey().x_only_public_key();
            PeerIdentity::from_pubkey(xonly).npub()
        } else {
            self.peer_display_name(src_addr)
        };

        let datagram =
            crate::app::AppDatagram::new(peer_npub, *src_addr, src_port, dst_port, payload);
        ...
        if let Err(e) = sender.send(datagram) {
            debug!(
                ...
                dst_port,
                error = %e,
                "Failed to deliver DataPacket to app port"
            );
        }
    }
